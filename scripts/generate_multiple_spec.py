...
if __name__ == '__main__':
    ...
    #cnn
    dim_x = [7]
    bounds_domain_x = [[(1, 11), (10, 1000), (0, 1), (0, 1), (10, 1000), (100, 1000), (2, 11)]]
    problem_name = ['cnn_cifar10']
    training_name = [None]
    type_kernel = [[PRODUCT_KERNELS_SEPARABLE, MATERN52_NAME, TASKS_KERNEL_NAME]]
    dimensions = [[8, 7, 8]]
    bounds_domain = [[[1, 11], [10, 1000], [0, 1], [0, 1], [10, 1000], [100, 1000], [2, 11],
                      [0, 1, 2, 3, 4]]]
    #old bounds: [[0.01, 1.01], [0.01, 2.1], [1, 21], [1, 201], [0, 1, 2, 3, 4]]
    n_training = [15]
    random_seed = range(1,10)
    n_specs = len(random_seed)
    type_bounds = [[0, 0, 0, 0, 0, 0, 0, 1]]
    x_domain = [[0, 1, 2, 3, 4, 5, 6]]
    number_points_each_dimension = [[6, 6, 11, 6, 1, 1, 1]]
    mle = [True]
    distribution = [UNIFORM_FINITE]
    parallel = [True]
    thinning = [10]
    ...
    same_correlation = [True]
    debug = [False]
    number_points_each_dimension_debug = [[10, 10, 10, 10, 10, 10, 10]]
    monte_carlo_sbo = [True]
    n_samples_mc = [5]
    n_restarts_mc = [5]
    n_best_restarts_mc = [0]
