...
class HutterGreedyPolicy(object):

    def __init__(self, dict_stat_models, name_model, problem_name, type_model='hutter', epsilon=0.1, total_iterations=100,
        ...
        for i in dict_stat_models:
            self.chosen_points[i] = list(dict_stat_models[i].raw_results['points'])
            self.evaluations_obj[i] = list(dict_stat_models[i].raw_results['values'])
    ...
    def get_parameters(self):
        for index in self.dict_stat_models:
            ...
            params = model.compute_posterior_params_marginalize(n_samples=self.n_samples, get_vectors=True)
    ...
    def choose_move_point(self):
        ...
        for t in range(self.n_epochs):
            ...
            move_model.add_observations(i + 1, data_new['value'],
                                        data_new['point'])
        ...
        params = move_model.compute_posterior_params_marginalize(
            n_samples=self.n_samples, get_vectors=True)
