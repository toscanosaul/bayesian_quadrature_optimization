...
class SDE(object):
    ...
    def ei_given_sample(self, sample, parameters_kernel, candidate_point):
        ...
        candidate_vector = np.zeros((len(self.domain_xe), 1))
        for j in range(len(self.domain_xe)):
            point = np.concatenate((candidate_point, np.array(self.domain_xe[j])))
            candidate_vector[j, :] = point
        cov_2 = self.gp.kernel.evaluate.evaluate_cross_cov_defined_by_params(
            parameters_kernel, candidate_vector, create_vector)
        weights_matrix = self.weights.reshape((len(self.weights),1))
        kron = np.kron(np.identity(n), weights_matrix)
        big_cov = np.concatenate((cov, np.dot(cov_2, kron)), axis=1)

        c = np.dot(weights_matrix.transpose(), big_cov)

        part_1 = np.dot(c.transpose(), cho_solve(chol, Z - bc * one))
        mc = bc + part_1
        ...
        Rc -= np.dot(c.transpose(), cho_solve(chol, c))
        one = np.ones((2 * n, 1))
        Rc += (1 - np.dot(c.transpose(), cho_solve(chol, one))) ** 2 / \
              (np.dot(one.transpose(), cho_solve(chol, one)))

        Zc = Z.reshape((len(Z),1))
        sigma_c = np.dot(Zc.transpose(), cho_solve(chol, Zc))
        sigma_c -= (bc ** 2) *  np.dot(one.transpose(), cho_solve(chol, one)) / (2.0 * n - 1)
    ...
    def evaluate_squared_error(self, control, environment, parameters_kernel):
        ...
        r = self.gp.kernel.evaluate.evaluate_cross_cov_defined_by_params(
            parameters_kernel, historical_points, new_point)
        one = np.ones(n)
        m1 = beta + np.dot(r.transpose(), cho_solve(chol, y - beta * one))
        M = np.concatenate((y, m1))
        M = M.reshape((len(M), 1))
        ...
        for j in range(len(self.domain_xe)):
            ...
            candidate_vector[j, :] = point
        R3 = self.gp.kernel.evaluate.evaluate_cross_cov_defined_by_params(
            parameters_kernel, candidate_vector, candidate_vector.transpose())
        R3SN = self.gp.kernel.evaluate.evaluate_cross_cov_defined_by_params(
            parameters_kernel, candidate_vector, historical_points.transpose())
        weights_matrix = self.weights.reshape((len(self.weights), 1))
        r3 = self.gp.kernel.evaluate.evaluate_cross_cov_defined_by_params(
            parameters_kernel, candidate_vector, new_point)
        ...
        one_matrix = np.ones((r.shape[0], r.shape[1]))
        ...
        R -= np.dot(e12.transpose(), cho_solve(cholE, e12))

        temp = (1 - np.dot(e12.transpose(), cho_solve(cholE, np.ones((n+1, 1))))) ** 2
    ...
    def get_environment(self, control, parameters_kernel):
        """
        See p.1142, eq. 15
        :param control:
        :return:
        ...
        values = []
        # do in parallel
        for point in self.domain_xe:
            value = self.evaluate_squared_error(control, point, parameters_kernel)
            values.append(value)

        j = np.argmax(np.array(values))
        return self.domain_xe[j]
    ...
    def mean_objective(self, candidate_point, parameters_kernel):
        ...
        sample = z * beta + solve
        # We just copy the code of ei_given_sample
    ...
    def optimize(self, **kwargs):
        return {'solution': self.iteration_algorithm(), 'value': 0}
