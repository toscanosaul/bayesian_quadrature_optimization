    ...
    for i in range(m):
       ...
       # setBikes=X[i]/len(temp)
        ...
        while (resT>0):
            ...
            for ind_index3 in index3:
                A[indx2[int(ind_index3)],1]+=setBikes
                A[indx2[int(ind_index3)],0]-=setBikes
...
def findBikeStation(state,currentBikeStation):
    ...
    dist=distancesBikeStations[int(currentBikeStation),:]
...
def unhappyPeople (T,N,X,m,data,cluster,bikeData,parLambda,nDays,A,poissonArray,timesArray,ind=None,randomSeed=None):
   ...
   # parLambda=parLambda.astype(int)
    ...
    for i in xrange(nTimes):
        ...
        if state[int(bikePickUp),1]==0:
            unHappy+=1
            continue
        indi=exponentialTimes[1,]
        timeUsed=randst.exponential(exponentialTimes2[int(bikePickUp),int(bikeDrop)])
        dropTimes.append((currentTime+timeUsed,bikeDrop))

        dropTimes=sorted(dropTimes, key=lambda x:x[0])
        
        state[int(bikePickUp),1]=state[int(bikePickUp),1]-1
        state[int(bikePickUp),0]=state[int(bikePickUp),0]+1
    return -unHappy
...
def generatePoissonParameters(nDays,nStations):
    ...
    for i in xrange(nDays):
        ...
        fil=os.path.join("problems/citi_bike_mt/NonHomogeneousPP2",fil)
        poiss=np.loadtxt(fil)
        for j in range(nStations):
            for k in range(nStations):
                lamb.append(poiss[j,k])
        parametersLambda[i]=np.sum(np.array(lamb))
    f=open(os.path.join("problems/citi_bike_mt/NonHomogeneousPP2","poissonDays.txt"),'w')
